...
## その31 arXiv 版パッケージ(2026-09-07)

- PNAS Nexus 投稿後、arXiv 用に `manuscript/arxiv_v1/` を新設。単一 `main.tex` = 本文(main.tex 投稿版と同一内容)+ 参考文献(`main.bbl` 同梱、arXiv は BibTeX を走らせないため必須)+ `\clearpage` 後に SI 本体を連結。SI 側は section/figure/table/equation のカウンタをリセットし S 接頭辞(S1–S8、Fig. S1、Table S1–S2)。SI 内部参照 "(Section 2.2 above)" → "(Section S2.2)" に修正。`longtable` パッケージ追加。
- コンパイル: pdflatex ×3(bbl 同梱)→ 33 頁、エラー 0、未定義参照 0、"claim-induced" 0 件。zip を展開した素の状態でも同結果を確認。
- `arxiv_v1_upload.zip`(35 ファイル、zip ルート直下に main.tex/main.bbl/refs.bib/figs/tables、PDF は含めない)をそのまま arXiv にアップロードする。
- メタデータ(README_arxiv.md): タイトルは投稿版と同一、abstract は `abstract_arxiv.txt`(1,602 字 / 246 語、上限 1,920 字)、Comments "21 pages main text + 12 pages supplementary information (single PDF), 7 figures + 1 SI figure"、主カテゴリ physics.soc-ph、クロスリスト cs.MA, cs.CL, nlin.AO。
- arXiv 公開時に GitHub リポジトリを public にする(ユーザ操作)。Data and code availability の記述と整合させる。

## その32 分量超過による差し戻し対応(2026-09-08)

- PNAS Nexus 編集部より Research Report の上限 12 頁超過で差し戻し(頁数の明示なし)。上限は abstract・significance・本文(Methods 含む)・図表キャプションを算入、参考文献は算入外。SI は査読対象・本文から引用必須・1 ファイル 5 MB 以下。
- 本文 detex 語数 9,654 → 8,759。結果・数値・図・主張は一切変更なし。移動・圧縮の内訳: Methods「Models and serving」「Prompts」を要約 1 節に統合(逐語プロンプト等は SI S10 へ)、「Cross-model calibration campaign」「Statistics」「Pre-registration timeline」を要約(全文は SI S10 へ逐語移動)、Section 3 を圧縮(例示コミュニティ・次数の詳細は SI S1 へ)、Discussion の要約 1 文とフリップ/分岐比の段落を削除(SI S8.5 に既載)、softmax・divisive normalization 段落を圧縮、Introduction 末尾の roadmap 段落・Section 4 の cell-by-cell 文・Fig. 3 キャプションを圧縮。
- SI: 節・表番号を S 接頭辞に統一(tocloft で目次幅調整)、新 S10「Extended Methods」追加、12 → 15 頁。本文からの参照は "SI Section S1/S9/S10"。
- 検査: 本文から消えた数値 14 件のうち SI に不在は「3.6」のみ(旧 Discussion の分岐比 1.8–3.6; 表 t_E_branching の最大値は 3.546 で 3.5 が正しく、旧本文の丸めが 1 桁ずれていた。削除により解消)。エラー 0、未定義参照 0、"claim-induced" 0。
- 成果物: `manuscript/submission_pnasnexus_20260908_resub/`(01 main 19 頁・02 SI 15 頁・source.zip・00 note)。`manuscript/arxiv_v1/` を同一ソースから再生成(33 頁、SI は p20 から、README の Comments を "19 pages main text + 14 pages supplementary information" に更新)。arXiv は未投稿なので v1 として今回版を上げる。
- 分量の目安: 誌の換算(6 頁 ≈ 4,000 語 + 50 文献 + 図 4 点)で参考文献 16 件・図 7 点なら 8,759 語は 12 頁内に収まる見込み。編集部の実測が不明のため、再差し戻し時は Introduction・Section 5.4 から更に約 300 語を削る案を用意。
- 参考文献の更新(2026-09-08、外部チェック + 自己確認): Arditi et al. → NeurIPS 2024 proceedings(神経情報処理学会ポスター頁で確認)、Sharma et al. → ICLR 2024(ICLR ポスター頁で確認、本文引用年 2023 → 2024)、De Marzo et al. → Science Advances 12, eaea6091 (2026), doi:10.1126/sciadv.aea6091、公開版タイトル "AI agents can coordinate via majority-following beyond human scale"(PubMed 42600034 / 報道記事で確認; 号番号は未確認のため記載せず)。bib キーは据え置き(DeMarzo2024/Sharma2023/Arditi2024)、arXiv 番号を note に保持。他 13 件は実在・書誌一致を確認済み。再投稿フォルダ・arXiv パッケージ・Overleaf(refs.bib)に反映、19/15/33 頁不変。
- 提出 PDF の頁数対策(2026-09-08): 著者テンプレート(11pt 一段組)では 19 頁のため、同一本文の二段組版 `tex/main_2col.tex`(extarticle 9pt・余白 1.8 cm・図は figure* で 0.42–0.85\textwidth・幅広の式 2 本を multline/gather に分割)を作成、参考文献込みで 12 頁。再投稿フォルダの 01 をこの二段組版に差し替え、一段組版は 01a として保存、source.zip に main_2col.tex を同梱。誌の上限は組版後の頁数だが、編集部の判定基準が不明なため PDF 自体を 12 頁以内にして曖昧さを除く。main_2col.tex は main.tex から機械的に派生(本文は同一)、今後 main.tex を直したら再生成する。
- 所属表記の追加(2026-09-08): 著者ブロックを「Makoto Fukushima / Research Division, Honda Research Institute Japan Co., Ltd., Wako, Saitama, Japan / Correspondence: makoto.fukushima@jp.honda-ri.com」に統一(main.tex・main_2col.tex・si.tex・arXiv main.tex・cover letter tex/md)。頁数不変(19/12/15/33)。Overleaf 同期済み。

## その33 arXiv 版の可読性改稿(v2 草稿、2026-09-12)

- 目的: 分野外の読者にも読める arXiv 版。対象は本文のみ(abstract〜Methods)。SI・図表・参考文献・全数値は不変(本文中の数値集合の前後一致を機械確認、SI 部分はバイト一致)。投稿済み PNAS Nexus 版のファイルは触らない。
- 構成の変更: Introduction を「問題(読める量の上限、正しい多数派からの誤合意)→ 二つの材料(crowding parameter が上限を網に変える、測定した agent は binary neuron)→ 量的な問い → 交絡と設計 → 人間への含意 → 道筋」の順に書き直し、末尾に用語表(Table 1、17 項目)を追加。Discussion 冒頭を「The reading bound does not decide a collective's fate on its own. Across three models it did so together with one more single-agent quantity.」で始め、検証内容を verified / predicted and not observed / observed as capacity dependence の 3 部に整理、末尾に Conclusion 節を新設。5.4 の節題を "A second model shows predicted transition points; a third lacks the assertion bias" に変更。
- 文の変更: 平均文長 37.6 → 33.3 語、60 語超の文 23 → 11、ダッシュ挿入句 44 → 0。Sec. 2 に地の文での写像・固定点・fold の説明、Sec. 4 の 250 語段落を 3 段落に分割、Sec. 5.1 を 4 段落に分割、Results 各段落に平易な要旨文を追加。
- abstract を書き直し(291 語 / 1,820 字、arXiv 上限内、"we found that"・"effectively" 維持、"Thus" で終える)。誌の 250 語上限は超えるため、誌の改訂時には別途短縮する。
- 成果物: `manuscript/arxiv_v2/`(main.tex・main.pdf 36 頁・arxiv_v2_upload.zip・abstract_arxiv.txt・README・CHANGES_v2.md)。zip の素展開コンパイル 36 頁・エラー 0 を確認。
- 用語表の移動(2026-09-12): 本文 Table 1 を SI の先頭節 S1「Glossary of terms used in the main text」(Table S1)へ移動。旧 SI S1–S10 は S2–S11 に繰り下げ、本文の参照(S1→S2 再解析、S9→S10 fidelity、S10→S11 Extended Methods)と SI 内部参照(S2.2→S3.2)を更新。Introduction 末尾は「A glossary of the terms used below is given in SI Section S1.」の 1 文。36 頁(本文 21・SI p22〜)、エラー 0、数値集合不変。zip 再生成・素展開コンパイル確認済み。
- 第 2・3 パス(2026-09-12): Introduction から結果の記述を全て除去(「as shown below」文、測定済みの形の記述、再解析の結果、道筋段落の結果要約)。節番号参照は道筋段落のみ。abstract で basin を初出時に定義(300 語 / 1,861 字)。36 頁・エラー 0・数値集合不変。arxiv_v2 一式・zip 更新(NAS manuscript/arxiv_v2/)。
- 第 4 パス(2026-09-13): abstract の message capacity を作用で説明(α 記号・指数則・α* = 0.435 を abstract から外し、遷移点は「fewer than 6.4 of their 31 sources」で表現、「generate the communication network from it」を追加)。292 語 / 1,803 字。Introduction 第 2 段落に「(the crowding parameter of that model)」を追加し用語の初出を保持。36 頁・エラー 0・本文数値集合不変(0.435 は本文 5 箇所に残存)。arxiv_v2 一式・zip・NAS 更新。
- Overleaf(2026-09-13): arXiv v2 を別プロジェクト「AI_agent_crowding_arxiv_v2」(project/6aa696659e11dc9ab2b4e11d)として zip から新規作成、36 頁でコンパイル確認。誌投稿版プロジェクト「AI_agent_crowding_manuscript」(6a92ee1cb96b463d5cbe0100)は据え置き。
- 用語整理と Overleaf 編集の取り込み(2026-09-14): 著者の Overleaf 編集(再解析節を「Measuring the response function of the experimental agent」の 3.1 に統合、Conclusion の "equivalent to")を反映し、3.2「The experimental agent」を追加、道筋の参照を Section 3 に修正、SI の節番号(3・4.2・4.2–4.4)を更新。「tilt」を廃し、場 h を binary neuron の閾値(符号反転)として 2.2 で定義(gain/threshold の区別を明記)。集団側の "threshold" は本文全体で "transition point"(表題の語)に統一、判定基準は "criterion"、K* は "critical degree"、hard-threshold unit は据え置き。SI 内の "threshold" は遷移点の意で残し、前書きに一文追加。abstract 300 語 / 1,856 字、Significance 119 語、37 頁、エラー 0、数値集合不変。Overleaf(arxiv_v2)・NAS・zip 更新。
- リズム改稿(2026-09-14): 著者の過去 3 報(2014 J Neurosci・2015 RSOS・2015 Sci Rep)から文体プロファイルを抽出し(`manuscript/arxiv_v2/style_profile_fukushima_2014-2015.md`)、Sections 2–5 と Conclusion を四拍段落(問い→手法→数値→解釈の締め)に組み直し。Results 小見出しを主張形に(4.2–4.6)、Discussion を過去形の要約で開始。Introduction・Methods・図キャプション・SI は不変。文長中央値 32→24 語、40 語超 30%→11%、文頭 We/This/Thus(/200)7/5/1→10/8/6。数値・引用集合は改稿前後で同一。38 頁、エラー 0。arxiv_v2 一式・zip・NAS・Overleaf(arxiv_v2)更新。同プロファイルを fukushima-writing スキルの規範 12 として提案(保存は著者操作)。
- 誌投稿先の変更(2026-09-15): PNAS Nexus は Editorial Board 段階で不採択(2026-09-14、「specialized、broader community への訴求不足」の scope 判断、査読なし)。候補比較(PRX Life・PLOS Complex Systems・PRE・Commun Phys・Collective Intelligence・AAMAS 2027・ICLR 2027・IEEE Access)の結果、著者判断で IEEE Access(判定平均 4 週、投稿から掲載 4–6 週、二値判定、APC $1,995)へ。
- arXiv v2 の追加改稿(2026-09-15): Introduction 冒頭に作業記憶容量(Cowan 2001; Luck & Vogel 1997)と選択的注意(Cherry 1953 カクテルパーティ効果; Treisman 1964 減衰)を引用。Discussion の Taniguchi et al. (2025) 文を修正(「remedy」の含意を除去、decentralized-Bayesian 枠組みの境界条件として記述、引用は括弧形、主語に置かない)。3 文献の実在を出版社記録で確認。38 頁・エラー 0・数値集合不変。arxiv_v2 一式・zip・NAS・Overleaf(v51)更新。
- IEEE Access 版(2026-09-15): `manuscript/ieee_access/` に作成。テンプレート ACCESS_latex_template_20260513(ieeeaccess.cls)。本文 I–VI(Methods は Appendix A、用語表 Appendix B、単一エージェント係数表と縮約写像検証 Appendix C、忠実度検査 Appendix D)、Acknowledgment に AI 利用開示、Biography(プレースホルダー)、17 頁二段組。Supplementary Material S1–S7(旧 SI S2, S5–S9, S11)を別 PDF 12 頁。abstract 249 語、キーワード 7。数値・引用集合は arxiv_v2 と同一。要記入: 経歴・写真・肩書、助成文、ORCID(0000-0002-8809-7892 はポータル入力)、graphical abstract(Fig. 1 + graphical_abstract.txt)。
- 追記(2026-09-15): arXiv v2 の Taniguchi 段落を 3 文・引用 1 回に短縮、SI の古い節番号参照 3 箇所を修正(S4「Section 4」→3.2 ×2、S5「Section 4.2」→4.4)。IEEE Access 版に著者写真(ポートレート切り出し)と 121 語の経歴(公開 CV から)を反映。両版とも NAS 更新。`manuscript/ieee_access/_old/` は旧プレースホルダーの退避先で削除可。
- IEEE Access 投稿(2026-09-15): 著者がポータルで投稿完了。Main Manuscript は単一 main.tex のフラットな LaTeX アーカイブ(サブフォルダ・hyperref なし、表 3 個を本文に展開、59 ファイル)+ 同内容の PDF、Supplementary Material は supp.pdf、graphical abstract は Fig. 1(PNG 300 dpi)+ 1 文。Acknowledgment 末尾に "The author declares no conflict of interest." を追加。subject: Computational and artificial intelligence / Systems, man, and cybernetics / Computers and information processing。Code Ocean・DataPort は見送り(GitHub リポジトリを参照)。
- arXiv パッケージ(2026-09-15): `manuscript/arxiv_v2/arxiv_v2_upload.zip` は IEEE 投稿と同一本文(README_arxiv.md に状態を追記、Comments に "Submitted to IEEE Access")。arXiv 投稿はユーザ操作。GitHub 更新指示 v2.2(`repo_prep/github_update_v22.md`)を作成(GX10 で実行)。
