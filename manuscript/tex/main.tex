...
\title{Claim-induced fields and message-capacity limits set the transition points of collective truth-finding in language-model networks}
...
\begin{abstract}
Human or large language model (LLM), an agent in discussion processes only a
few of the others' contributions, bounded by cognition, context, or cost.
...
$\alpha$: each agent accepts a further input with probability $e^{-\alpha r}$
...
vanish at $\astar = 0.435$ (fan-in 6.4 of 31 sources). In 1{,}414 episodes
with assigned opinions the prediction failed: correct-side outcomes stayed
below 50\% of episodes from every start, only 28--45\% from
75\%-correct majorities. The failure traces to the claim-induced field: the
experimental claims' fields lay far below the
calibration mean; with each claim's own field the same weights reproduce the
outcomes. Reversing the claims' wording identified its origin:
wording tilts every reading toward a claim's assertion, not its truth. On a
second 8B model the pipeline predicted claim-dependent bistability; basin
...
step: the claim-induced field, which did not carry over from the calibration
claims to the experimental ones, and the eight-round horizon, which is short
...
$h = c_0 + \theta/2$ (the persistence indicator decomposes into a constant
field $\theta/2$ plus a symmetric term), the per-input swing
...
The diagnosis localizes the failure to the claim-induced field. Replaying
...
through the surrogate, not directly.
...
across claims, and identifies the claim-induced field as the quantity that
...
grid (SI). For this model, the collectives follow the assertion, not the
truth --- which reframes the social reading of Section~\ref{sec:results}: the
...
qualitatively different outcome (Fig.~\ref{fig:generality}). For
...
on the precondition that the claim-induced field and any model-specific
...
because the claim-induced fields placed three of the four claims in the
...
reading budget: the router, not the model, implements the acceptance kernel,
a division of labor the measurements themselves selected
...
final fraction and one claim outside both coupling rules remaining.
