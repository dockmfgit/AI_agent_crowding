...
\author{\uppercase{Makoto Fukushima}\authorrefmark{1}}

\address[1]{Research Division, Honda Research Institute Japan Co., Ltd., Wako, Saitama 351-0188, Japan}
\tfootnote{This work was supported by Honda Research Institute Japan Co., Ltd.}

\markboth
{Fukushima: Message Capacity and Claim Wording Set the Transition Points of Collective Truth-Finding}
{Fukushima: Message Capacity and Claim Wording Set the Transition Points of Collective Truth-Finding}

\corresp{Corresponding author: Makoto Fukushima (e-mail: makoto.fukushima@jp.honda-ri.com).}

\begin{abstract}
Human or large language model (LLM), an agent in a discussion reads only a few of the others' contributions, bounded by cognition or cost. LLM collectives can settle on a wrong consensus even from a correct majority; we ask how far that bound decides the outcome. We model the bound with one number, the message capacity, and generate the network from it. Over 31,824 randomized queries, we found that an 8-billion-parameter model's judgment of a claim effectively reduces to a logistic function of a weighted sum of its inbox, the update rule of a stochastic binary neuron with divisively normalized weights. From these weights and the network's degree statistics alone, the wrong consensus should become unreachable once agents read on average fewer than 6.4 of their 31 sources. In 1,414 episodes the prediction failed: the correct side won in under 50\% of episodes from every start, 28--45\% from 75\%-correct starts. The failure traces to the field, the threshold a claim's wording sets before any message is read: with each claim's own field the same weights reproduce the outcomes, and reversing the wording showed the threshold follows what a claim asserts, not whether it is true. On a second 8B model, transition points appeared where computed and an eight-claim calibration matched in 15 of 16 conditions. At 70B the assertion bias is not detected. Thus a collective's fate is largely set by two single-agent measurements: the threshold a claim's wording sets, and the message capacity that sets the transition point.
\end{abstract}

\begin{keywords}
Collective intelligence, language models, mean-field theory, multi-agent systems, network science, opinion dynamics, phase transitions.
\end{keywords}
...
\PARstart{W}{hether} human or large language model (LLM), an agent in a discussion
...
\cite{Cowan2001,LuckVogel1997}, and selective attention narrows the intake
further: a listener in a crowded room follows one conversation and retains
almost nothing of the others \cite{Cherry1953}, because unattended streams
are attenuated before they reach the capacity-limited stage
\cite{Treisman1964}. In online discussions the same limit appears as
divided attention capping how many contributions a participant actually
processes \cite{HodasLerman2012}. For LLM agents the bound is set by
...
now studied as statistical-mechanical systems \cite{El2026}, and whether
...
a network. In a model of synaptic resource allocation \cite{SciRepCrowding},
...
and collective bias are established phenomena \cite{Ashery2025, Flint2026}:
a mean-field consensus condition has been assembled from single-token
acceptance rates measured on isolated models \cite{DeNobili2026}, and a
mean-field account of population-size-dependent collective bias from
logit-extracted single-agent policies \cite{Flint2026}. These constructions
...
the truth-finding setting, El et al.~\cite{El2026} showed that a kinetic Ising model
...
used below is given in Appendix~\ref{app:glossary}.
...
We generate the network by the process of Fukushima~\cite{SciRepCrowding}. Each
...
\cite{SciRepCrowding} with one substitution: the hard-threshold unit
...
after eight rounds (Appendix~\ref{app:methods}). Note that no hard-threshold nonlinearity is
...
$\bar\beta = 1$) (Fig.~\ref{fig:theory}; Appendix~\ref{app:fold}). For the measured variant-A
...
sets of El et al.~\cite{El2026}; the interior unstable branch (open symbols) vanishes
...
The natural first test requires no new data. El et al.~\cite{El2026} released full
logs of 6{,}400 objective-question communities (four models, 40 questions,
ten graphs per question, four episodes) \cite{El2026data}. We therefore
...
bit (Supplementary Material Section~S1).
...
Claims came from CLIMATE-FEVER \cite{Diggelmann2020}. A two-stage
...
under the full scaffold (Appendix~\ref{app:methods}), and, as Section~\ref{sec:results} shows,
...
Three measured facts carry the predictions (Fig.~\ref{fig:neuron}; per-$k$ coefficients and interval estimates in Fig.~\ref{fig:si_single}, Appendix~\ref{app:single}).
...
$-0.39$ $[-0.48, -0.33]$ at $k = 12$ (Fig.~\ref{fig:si_single}). Note that separate fits
at fixed $k$ do not identify these coefficients (Appendix~\ref{app:methods}).
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.8\textwidth]{fig3_neuron}
...
the criterion ($\delta \in \{0.05, 0.2\}$) is reported in Supplementary Material Section~S2.
...
two of them to within the resolution of the seeding grid (Supplementary Material Section~S3).
...
stance \cite{Chuang2024}. Where the true position is the affirming one, a
...
final estimation procedure (Appendix~\ref{app:methods}). Their agreement with the collectives
...
the campaign (the predictions reported here, recomputed with the estimation procedure of Appendix~\ref{app:methods}, leave the selected claims' classes unchanged in 15 of
...
intermediate end states (per-cell outcome counts in Supplementary Material Section~S5). (b)~The
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig7_calibration}
...
$\alpha = 0.30$) are listed in Supplementary Material Section~S5.}
...
\cite{Flint2026}; in human information sharing, divided attention limits
message visibility in the same direction \cite{HodasLerman2012}. The
...
Bayesian inference over exchanged messages \cite{Taniguchi2024} treat
...
panels under peer pressure \cite{HuQu2026}. The polarity channel measured
...
resolved. An abliterated (refusal-direction-ablated, \cite{Arditi2024})
...
the preference data used for RLHF \cite{Sharma2023}; here the agreement is
...
Relative to El et al.~\cite{El2026}, their graphs were given, ours are generated;
...
size thresholds \cite{Flint2026}, convergent with the model dependence
...
consensus-scaling observations \cite{DeMarzo2024}, $(\log N)/\alpha$
...
neuroscience as divisive normalization \cite{CarandiniHeeger2012}. As in
...
one-step regression (Supplementary Material Section~S2), which could reflect either the messages or
...
\appendices
\section{Experimental Methods}\label{app:methods}
...
variants of the polarity-origin probe are given in Supplementary Material Section~S7. Every
call's prompt hash, seed, raw response, and parse result are logged to JSONL.

\subsection{Claims and calibration}

Claims are drawn from CLIMATE-FEVER \cite{Diggelmann2020} (SUPPORTS and
...
any collective run. Full claim texts, rewrites, and calibration values are in Supplementary Material Section~S3.
...
the realized graphs (RMS difference 0.04 in the final fraction). A fidelity check with five-sample majority voting (Appendix~\ref{app:fidelity}) gave
...
$\delta \in \{0.05, 0.2\}$ and undecided-exclusion sensitivity analyses in Supplementary Material Section~S2.
...
replicates (256 episodes; Supplementary Material Section~S7).
...
a claim share their field; resample counts in Supplementary Material Section~S7).
...
the construction of Fig.~\ref{fig:neuron}, and the bootstrap retention counts are in Supplementary Material Section~S7.
...
fixed in the pre-registered design (Supplementary Material Section~S7).
...
dataset of El et al.~\cite{El2026} is not redistributed and is available from its
original source \cite{El2026data}. Layout: \texttt{scripts/} (measurement, engines, analysis),
...
Table~\ref{tab:terms} collects the terms used in the article.
\begin{table*}[!t]
\caption{Glossary of the terms used in the article.}
\label{tab:terms}
\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.26\textwidth}p{0.70\textwidth}@{}}
\toprule
Term & Meaning \\
\midrule
...
\section{Single-Agent Measurement and Reduced-Map Validation}\label{app:single}

\subsection{Response coefficients}
Table~\ref{tab:coeffs} lists the pooled response coefficients of llama3.1:8b for both prompt variants.
\begin{table}[!t]
\caption{Pooled response coefficients (llama3.1:8b; claim-clustered 95\% intervals).}
\label{tab:coeffs}
\centering\small
\resizebox{\columnwidth}{!}{\input{tables/t_stage1_coeffs}}
\end{table}

\subsection{Attenuation-form comparison}\label{app:decay}
Table~\ref{tab:decay} compares the candidate attenuation forms by log-likelihood, AIC, and claim-split cross-validated log-likelihood.
\begin{table}[!t]
\caption{Attenuation-form comparison (five-fold cross-validation with folds split by claim).}
\label{tab:decay}
\centering\small
\resizebox{\columnwidth}{!}{\input{tables/t_decay}}
\end{table}
...
$c_0 + k\beta_F$ and $\beta_T - \beta_F$. The per-$k$ coefficients in Table~\ref{tab:perk} are
...
\begin{table*}[!t]
\caption{Identified per-$k$ coefficients (claim-clustered bootstrap percentiles, 200 resamples).}
\label{tab:perk}
\centering\small
{%
\begin{tabular}{rcccc}
...
\subsection{Reduced-map validation}\label{app:fold}
...
form~(1). Synthetic sets use $\beta_T = -\beta_F = \bar\beta$ (Table~\ref{tab:fold}).

\begin{table*}[!t]
\caption{Fold of the full map $F$ versus the two closure levels of the reduced map $G$ for seven synthetic and four measured coefficient sets.}
\label{tab:fold}
\centering\small
...
Section~\ref{sec:measurement} quotes the folds of the full map $F$ (first numeric
...
the pooled Stage 1 fits (Section~\ref{sec:measurement}). The divisive $\astar$ values
use $\gamma$ profiled separately per prompt variant ($\gamma_A = 0.526$,
$\gamma_B = 1.318$); Fig.~\ref{fig:neuron} and the per-claim response
functions of the collective experiments use the pooled profile $\gamma = 0.9$
(Appendix~\ref{app:decay}). Fig.~\ref{fig:theory} plots Table~\ref{tab:fold}.
...
\section{Fidelity Check}\label{app:fidelity}
...
The author used Anthropic's Claude (Claude Code) for code generation and
debugging, execution of the pre-specified experiment pipelines, drafting
and editing of text throughout Sections~I--VI, the appendices, and the
supplementary material, and literature search. The research questions,
theory, experimental designs, pre-registered predictions, and
interpretations are the author's; all AI-assisted text was reviewed,
revised, and fact-checked by the author, and every reported number is
recomputed from the logged data by the scripts in the public repository.
The language models studied here (llama3.1:8b, qwen3:8b, llama3.3:70b) are
the subject of the research, not writing tools. The author declares no
conflict of interest.

\bibliographystyle{IEEEtran}
\bibliography{refs}

\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{author_photo}}]{Makoto Fukushima}
received the B.S.\ degree in physics from Waseda University, Tokyo, Japan, in 1997, and the Ph.D.\ degree in psychology from the University of Chicago, Chicago, IL, USA, in 2008. He was a Postdoctoral Fellow with the National Institute of Mental Health, Bethesda, MD, USA, a Research Scientist with the RIKEN Center for Brain Science, Wako, Japan, and a Manager of neurotechnology research with Deloitte Analytics R\&D. Since 2025, he has been a Principal Scientist with the Research Division, Honda Research Institute Japan Co., Ltd., Wako, Japan. His research interests include systems neuroscience, the neural coding of communication signals, and collective behavior in networks of neurons and artificial agents. He received the NIH Fellows Award for Research Excellence in 2012 and 2014.
\end{IEEEbiography}
